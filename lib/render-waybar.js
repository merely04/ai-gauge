...
function providerLabel(provider, tokenSource) {
  ...
  if (provider === 'ollama-cloud' || tokenSource === 'ollama-cloud') return 'Ollama Cloud Usage';
...
// Normal polling is 60s; this only trips after fetches have been failing for a
// while (network/VPN down), which is exactly when a low % is untrustworthy.
const STALE_THRESHOLD_SECONDS = 600;

// Returns a short elapsed-time label ("12m", "1h 5m") when fetchedAt is older
// than the stale threshold, else null.
function staleAgeLabel(fetchedAt, now = Date.now()) {
  if (!fetchedAt || fetchedAt === 'null') return null;
  const t = new Date(fetchedAt).getTime();
  if (!Number.isFinite(t)) return null;

  const ageSec = Math.floor((now - t) / 1000);
  if (ageSec < STALE_THRESHOLD_SECONDS) return null;
...
// Codex per-model buckets (GPT-5.5 / Spark) are enforced separately from the
// account-wide 5h/weekly windows, so a low account % can coexist with a real
// "usage limit reached". Surface them so the tooltip explains the mismatch.
function buildCodexModelLines(perModel, limitReached, now) {
  let out = '';
  if (limitReached === true) {
    out += '\n⚠ Account limit reached';
  }
  if (!Array.isArray(perModel) || perModel.length === 0) return out;

  for (const m of perModel) {
    const name = m?.name ?? m?.id ?? 'Model';
    let line = `\n· ${name}:`;
    const five = Number(m?.five_hour?.utilization);
    if (Number.isFinite(five)) line += `  ${Math.round(five)}%`;
    const seven = Number(m?.seven_day?.utilization);
    if (Number.isFinite(seven)) line += ` · ${Math.round(seven)}%w`;
    if (m?.limit_reached === true) {
      line += ' ⚠ limit reached';
      const reset = m?.five_hour?.resets_at ?? m?.seven_day?.resets_at;
      const r = formatDurationLong(reset, now);
      if (r) line += ` (resets in ${r})`;
    }
    out += line;
  }
  return out;
...
export function render(data, updateState = {}, now = Date.now()) {
  ...
  tooltip += '\n───────────────';
  const staleAge = staleAgeLabel(data?.meta?.fetchedAt, now);
  ...
  tooltip += buildCodexModelLines(data?.per_model, data?.codex_limit_reached, now);
  ...
  if (secondary) {
    ...
    tooltip += buildCodexModelLines(secondary?.per_model, secondary?.codex_limit_reached, now);
