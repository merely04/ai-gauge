...
describe('parseRegistryResponse — latest format validation', () => {
  test('rejects javascript: URI in latest', () => {
    expect(parseRegistryResponse({ 'dist-tags': { latest: 'javascript:alert(1)' } })).toEqual({
      error: 'invalid-version',
    });
  });

  test('rejects shell injection attempts in latest', () => {
    expect(parseRegistryResponse({ 'dist-tags': { latest: "0'; rm -rf /" } })).toEqual({
      error: 'invalid-version',
    });
  });

  test('rejects non-semver string in latest', () => {
    expect(parseRegistryResponse({ 'dist-tags': { latest: 'banana' } })).toEqual({
      error: 'invalid-version',
    });
  });

  test('accepts valid prerelease version', () => {
    expect(parseRegistryResponse({ 'dist-tags': { latest: '2.0.0-beta.1' } })).toEqual({
      latestVersion: '2.0.0-beta.1',
    });
  });

  test('accepts valid build metadata version', () => {
    expect(parseRegistryResponse({ 'dist-tags': { latest: '1.5.0+sha.abc123' } })).toEqual({
      latestVersion: '1.5.0+sha.abc123',
    });
  });

  test('strips BOM from JSON string input', () => {
    expect(parseRegistryResponse('\uFEFF{"dist-tags":{"latest":"1.0.0"}}')).toEqual({
      latestVersion: '1.0.0',
    });
  });
});
