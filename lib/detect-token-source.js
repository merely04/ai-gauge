...
const PRIORITY = { opencode: 0, pi: 1, codex: 2, 'claude-code': 3 };

// mtime semantics:
//   mtime > 0   — known file modification time (epoch ms)
//   mtime === 0 — UNKNOWN recency. Two cases:
//                  (a) Keychain-backed claude-code (no filesystem mtime exists)
//                  (b) statSync() failed for a file that does exist
// When sorting candidates, mtime=0 sources lose ties to known-recent files.
// This means Keychain-backed claude-code with no file fallback will always
// lose to a recently-touched opencode/codex file. This is intentional:
// if the user just authenticated with another tool, that tool wins.
...
// Classify a readJsonFile() rejection into a stable tag.
// Probes return this in `errorKind` so the detector can tell
// "file missing" apart from "file present but unreadable/corrupt".
//   'enoent'    — file does not exist
//   'malformed' — file exists but JSON parse failed
//   'eperm'     — permission denied (file may or may not exist)
//   'unknown'   — any other I/O error
function classifyReadError(err) {
  if (err?.code === 'ENOENT') return 'enoent';
  if (err?.name === 'SyntaxError' || /JSON/i.test(err?.message ?? '')) return 'malformed';
  if (err?.code === 'EACCES' || err?.code === 'EPERM') return 'eperm';
  return 'unknown';
}
...
async function probeClaudeCodeFile(credsFile, isTokenValidImpl, fsImpl) {
  ...
  try {
    ...
    const errorKind = classifyReadError(err);
    if (errorKind === 'malformed') {
      return { exists: true, valid: false, mtime: getMtime(credsFile, fsImpl), errorKind };
    }
    return { exists: false, valid: false, mtime: 0, errorKind };
  }
...
async function probeOpencodeOnePath(path, isTokenValidImpl, fsImpl) {
  ...
  try {
    ...
    const errorKind = classifyReadError(err);
    if (errorKind === 'malformed') {
      return { exists: true, valid: false, mtime: getMtime(path, fsImpl), errorKind };
    }
    return { exists: false, valid: false, mtime: 0, errorKind };
  }
...
async function probeOpencode(deps) {
  ...
  const primary = await probeOpencodeOnePath(paths.opencodePrimary, isTokenValidImpl, fsImpl);
  if (primary.exists || (primary.errorKind && primary.errorKind !== 'enoent')) {
    return { source: 'opencode', ...primary };
  }
  const mac = await probeOpencodeOnePath(paths.opencodeMac, isTokenValidImpl, fsImpl);
  return { source: 'opencode', ...mac };
}
...
async function probeCodex(deps) {
  ...
  try {
    ...
    const errorKind = classifyReadError(err);
    if (errorKind === 'malformed') {
      return { source: 'codex', exists: true, valid: false, mtime: getMtime(paths.codex, fsImpl), errorKind };
    }
    return { source: 'codex', exists: false, valid: false, mtime: 0, errorKind };
  }
...
// Pi auth.json is an object keyed by provider name; each value is a credential
// block. isPiBlockUsable() + this key set MUST stay identical to lib/credentials-pi.js.
const SUPPORTED_PI_PROVIDERS = ['anthropic', 'openai-codex', 'github-copilot', 'openrouter', 'zai'];

function isPiBlockUsable(block, isTokenValidImpl) {
  if (!block || typeof block !== 'object') return false;
  if (block.type === 'oauth') {
    if (typeof block.access !== 'string' || !block.access) return false;
    const expires = block.expires;
    if (expires === undefined || expires === null) return true;
    return isTokenValidImpl(expires);
  }
  if (block.type === 'api_key') {
    return typeof block.key === 'string' && block.key.length > 0;
  }
  return false;
}

async function probePI(deps) {
  const { paths, fsImpl, isTokenValidImpl } = deps;
  let data;
  try {
    data = await readJsonFile(paths.pi, fsImpl);
  } catch (err) {
    const errorKind = classifyReadError(err);
    if (errorKind === 'malformed') {
      return { source: 'pi', exists: true, valid: false, mtime: getMtime(paths.pi, fsImpl), errorKind };
    }
    return { source: 'pi', exists: false, valid: false, mtime: 0, errorKind };
  }
  const valid =
    !!data &&
    typeof data === 'object' &&
    SUPPORTED_PI_PROVIDERS.some((key) => isPiBlockUsable(data[key], isTokenValidImpl));
  return { source: 'pi', exists: true, valid, mtime: getMtime(paths.pi, fsImpl) };
}
...
function buildPaths(env) {
  ...
  return {
    ...
    codex,
    pi: join(home, '.pi', 'agent', 'auth.json'),
  };
}
...
export async function detectTokenSource(deps = {}) {
  try {
    ...
    const candidates = probes.filter((p) => p.exists && p.valid);
