...
const zaiAdapter = {
  ...
  parseResponse(json, responseStatus) {
    ...
    let fiveHourBucket = tokenLimits.find((limit) => limit.unit === 3) ?? null;
    let weeklyBucket = tokenLimits.find((limit) => limit.unit === 6) ?? null;

    const needsFallback = tokenLimits.length > 0 && !tokenLimits.some((limit) => limit.unit != null);
    if (needsFallback) {
      const sorted = [...tokenLimits].sort((a, b) => (a.nextResetTime ?? 0) - (b.nextResetTime ?? 0));
      fiveHourBucket = fiveHourBucket ?? sorted[0] ?? null;
      weeklyBucket = weeklyBucket ?? (sorted.length > 1 ? sorted[1] : null);
    }
