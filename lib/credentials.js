...
function extractOpenCodeCopilot(data) {
  const copilot = data?.['github-copilot'];
  if (copilot?.type !== 'oauth' || !copilot?.refresh) return null;

  return {
    provider: 'copilot',
    token: copilot.refresh,
    expiresAt: copilot.expires && copilot.expires > 0 ? copilot.expires : null,
    enterpriseUrl: copilot.enterpriseUrl ?? null,
  };
}
  ...
  for (const path of [primaryPath, macPath]) {
    try {
      ...
      return {
        ...
        copilotSecondary: extractOpenCodeCopilot(data),
      };
    } catch (err) {
      if (err?.code !== 'ENOENT') {
