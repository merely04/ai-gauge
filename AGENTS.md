...
5. **Patches waybar `config.jsonc` via `lib/patch-waybar-config.py`** — invoked by `_patch_waybar_config` with `AI_GAUGE_WAYBAR_BIN` / `AI_GAUGE_MENU_BIN` env vars holding absolute binary paths (resolved earlier in `cmd_setup_linux` via `command -v` + `resolve_path`). The patcher inserts the module on first run and **migrates** stale paths (legacy bare names like `ai-gauge-waybar`, or any prior absolute path) to the current resolved binary on subsequent runs. Absolute paths are mandatory because waybar freezes `PATH` at startup — bare command names break silently after `bun add -g` / `mise` upgrades that move symlinks. The patch looks for the literal string `"custom/notification-silencing-indicator",` to anchor insertion; on non-Omarchy configs the module array insertion may silently no-op while the module definition still gets appended before the final `}`.
...
- **Setup's waybar JSON patch is fragile** — it uses string replacement on `config.jsonc` (not a real JSON parser, since jsonc allows comments). Logic lives in `lib/patch-waybar-config.py` with `test/patch-waybar-config.test.js` covering 8 cases (insert, migrate from bare names, migrate from stale absolute paths, noop, env-var validation, comment preservation). Test changes against a real Omarchy/Hyprland waybar config.
